...
class Database:
	...
	def insertUser(self,user):
		self.cursor.execute("""INSERT INTO [dbo].[Users] ([ID],[IDStr],[creationDate]) VALUES(?,?,?)""",
		(user.ID,user.IDstr,user.creationDate)
		)
	def instertListofUsers(self,users):
		for i in range(0,len(users)):
			self.insertUser(users[i])
	def insertTweet(self,tweet):
		self.cursor.execute("""INSERT INTO [dbo].[Tweet] ([ID],[IDStr],[containsVideo],[numberOfPictures],[listOfHashtags],[time],[posterID],[retweets]) VALUES(?,?,?,?,?,?,?,?)""",
		(tweet.ID,tweet.IDstr,tweet.contains_videos,tweet.num_photos,tweet.list_of_hashtags,tweet.time,tweet.posterID,tweet.retweets)
		)
